...
function checklista() {
    ...
    if (line.length == 0){
        ...
        $('#ModalMsg').text("Error: lista form empty.");
        playError();
        return;
    }
    if (sk_live.length == 0) {
...
function clearCVV() {
 ...
 //   var x = document.getElementsByClassName("live_cvv");
  ...
  // Use like:
  removeElements( document.querySelectorAll(".live_cvv") );
...
function clearCCN() {
  ...
  //  for(var i=0; i < y.length; i++){
   //     y[i].remove();
    ...
    const removeElements = (elms) => elms.forEach(el => el.remove());
    // Use like:
    removeElements( document.querySelectorAll(".live_ccn") );
...
function clearDead() {
   ...
   // for(var i=0; i < z.length; i++){
    ...
    const removeElements = (elms) => elms.forEach(el => el.remove());
// Use like:
removeElements( document.querySelectorAll(".dead") );
...
function playLiam() {
    var liam = new Audio('./assets/sfx/liam.mp3');
    liam.play();
}
function playError() {
...
function playCcn() {
    var ccn = new Audio('./assets/sfx/ccn.mp3');
    ccn.play();
}
function playCvv() {
    var cvv = new Audio('./assets/sfx/cvv.mp3');
    cvv.play();
}
...
var aktuell = '<h4><b><i class="fa fa-gear fa-spin" style="font-size:24px"></i> Reloading...</b></h4>';
$('#status').html(aktuell);
}
function xpStartup() {
    var audio = document.getElementById("xpStartup");
    audio.play();
}
function xpShutdown() {
    var audio = document.getElementById("xpShutdown");
    audio.play();
}
function playStart() {
  ...
  setTimeout(function(){
    var audio = document.getElementById("startchk");
    audio.play();
  }, 2000);

}
function playLian() {
    var lian = new Audio('./assets/sfx/lian.mp3');
    lian.play();
}
...
function darkside(p1, tr) {
    ...
    if (p1 != "" && p1 != null) {
        ...
        for (var k = 1; k <= ccghm; k++) {
            if (p1 == 'rnd') p = c[Math.floor(Math.random() * (mxlista + 1 - 2)) + 2];
            else p = p1;
            var cn = chkCard(p);
            for (var i = tr; i >= 1; i--) {
...
function cleanText() {
    var confirmar = confirm("Do you want to remove the generated lista?");
